...
module Demo.SingleThread where
...
run cfg n = do logChan <- newChan
               logThread <- emptyLogDB >>= forkIO . logger cfg logChan
               chans <- replicateM n newChan
               threads <- (forkIO . worker cfg logChan chans) `mapM` chans
               getLine
               mapM_ killThread (logThread:threads)

logger :: Config -> Chan Log -> LogDB -> IO ()
logger cfg logChan logDB 
  = do log <- readChan logChan
       time <- getTime
       putStr ("Log time: ")
